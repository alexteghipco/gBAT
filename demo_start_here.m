...
beh = readtable('/home/alex/Downloads/abcBeh.csv');
subs = beh.Subject;
moca  = beh.TotalScore;
age = beh.Age;
...
for i = 1:length(subs)
    disp(num2str(i))
    try
        f = ls([pwd '/mri/smwp1' subs{i} '.nii.gz']);
        f = f(1:end-1);
        tmp = load_nifti(f);
        gm(:,:,i) = tmp.vol(:);
    catch
...
for i = 1:length(subs)
    disp(num2str(i))
    try
        f = ls([pwd '/mri/smwp2' subs{i} '.nii.gz']);
        f = f(1:end-1);
        tmp = load_nifti(f);
        wm(:,:,i) = tmp.vol(:);
    catch
...
tivd = readtable('/home/alex/Downloads/TIVDataABC.xlsx');
...
% load in the atlas
roiin = load_nifti('rjhu_resampled_to_FSL_fixed.nii');
...
kid = 1:132; % this is ABC folks (i.e., non-covid positive cohort)
for i = 1:height(labs) % loop over ROIs
    ...
    if labs.Var4(i) == 1 % if GM ROI
        ...
        morph(:,i) = mean(tmp,2); % get mean across voxels for the ROI-level morphometry, for each participant
