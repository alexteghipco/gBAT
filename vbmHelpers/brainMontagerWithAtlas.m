function [im,cont,m1,m2,uLim] = brainMontagerWithAtlas(inFU,inFO,inAt,sls,uBin,uLim,uCmpNm,uThr,backClr,maskOF,oBin,oLim,oCmpNm,oThr,oAlpha,vAng,contr,useRawLims,roi,gmThresh,anTail)
...
tmpa.vol = double(niftiread(inAt));
...
if isempty(uCmpNm)
    ...
    if contr
        if ~roi
            ...
            unROI = unique(tmpa.vol);
            id = find(unROI == 0);
            unROI(id) = [];
            %l = length(unROI);
            %l = ;
            if strcmpi(anTail,'neg')
                ic = 'on';
                if isempty(uLim)
                    uLim = [min(tmp.vol(:)) 0];
                    id = find(tmp.vol > 0);
            ...
            if isempty(uLim)
                uLim = [min(tmp.vol(:)) max(tmp.vol(:))];
            end
            [uCmpNm,uCData,ucbData,~,~] = colormapper(tmp.vol(:),'colorBins',uBin,'colormap',uCmpNm,'limits',uLim,'invertColors',ic);
        end
        % if l > 1
        %     [uCmpNm,~,~,~,~] = colormapper([1:l],'colorBins',l,'colormap',uCmpNm);
        % else
...
if contr
    ...
    for i = 1:length(sls)
        ...
        if ~roi
            ...
            l = length(unROI);
            uCData2 = reshape(uCData,[size(tmp.vol),3]);
            for j = 1:l
                hold on
                rid = find(tmpa.vol == unROI(j));
                tmpp = tmpa.vol(:,:,sls(i));
                %tmpp2 = uCData2(:,:,sls(i),:);
                id = find(tmpp ~= unROI(j));
                tmpp(id) = 0;
                if tmp.vol(rid) ~= 0
                    [~,cont{i,j}] = imcontour(tmpp);
                    cont{i,j}.LevelList = lvls;
                    cont{i,j}.LineWidth = 2;
                    cont{i,j}.LineColor = unique(uCData(rid,:),'rows');%[0 0 0];
                end
                if ~isempty(vAng2)
                    view(vAng2)
                end
    ...
    m2 = montage(tmp2.vol,'DisplayRange',[olins(1) olins(end)],'Indices',sls,'Interpolation','bilinear');
